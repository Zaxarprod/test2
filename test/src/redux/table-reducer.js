...
export const tableReducer = (state = initialState, action) => {
    switch (action.type) {
        case ADD_USER:
            ...
            return {
                ...state,
                table: [action.user, ...state.table],
                filterTable: state.filterTable?[action.user, ...state.filterTable]:state.filterTable,
            }
        case CURRENT_USER:
            return {
                ...state,
...
export const addUserTC = (user) => {
    return async (dispatch) => {
        let data = await axios.post('http://www.filltext.com/?rows=1000&id=%7Bnumber%7C1000%7D&firstName=%7BfirstName%7D&delay=3&lastName=%7BlastName%7D&email=%7Bemail%7D&phone=%7Bphone%7C(xxx)xxx-xx-xx%7D&address=%7BaddressObject%7D&description=%7Blorem%7C32%7D', user)
        debugger
